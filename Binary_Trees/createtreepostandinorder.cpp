...
Node*createtreepostandin(map<int,int>&mp,int*pre,int*in,int &preindex,int instart,int inend,int n){
        // base case
       ...
       // this map pos tak o(1)
        int pos=mp[element];
        root->left=createtreepostandin(mp,pre,in,preindex,instart,pos-1,n);
        root->right=createtreepostandin(mp,pre,in,preindex,pos+1,inend,n);
        return root;
}
int main(){
...
Node*ans=createtreepostandin(mp,pre,in,preindex,instart,inend,n);
